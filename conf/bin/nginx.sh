...
if [ -d "$NGINX_HOME/bin" ]
then
    if [ -f "$NGINX_HOME/bin/t-alipay-tengine" ];then
...
if [ -z "$CRONOLOG_HOME" ];then
    CRONOLOG_HOME="/opt/taobao/install/cronolog"
...
# kill previous nginx processes after replace HOME
