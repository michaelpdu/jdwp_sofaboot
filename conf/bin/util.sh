...
function get_confreg()
{
  ...
  if [ -n "${CONFREG_URL}" ]; then
    ...
    do
      ...
      confreg_datacenter=${key##*.}

      # compare case-insensitive
      lower_datacenter=`echo "$datacenter_name" | tr -s '[:upper:]' '[:lower:]'`
      lower_confreg_datacenter=`echo "$confreg_datacenter" | tr -s '[:upper:]' '[:lower:]'`
      echo "lower_datacenter=$lower_datacenter"
      echo "confreg_datacenter=$lower_confreg_datacenter"
      if [ "$lower_datacenter" == "$lower_confreg_datacenter" ]; then
        echo "using confreg $value"
        RESULT=$value
        return
      fi
    done
...
# load deploy-time from server.conf
# if user passed -P params, they should be in server.conf by now
function load_deploy_var() {
    if [ -z "$server_conf" ];then
        # default path
        server_conf=/home/admin/conf/server.conf
    fi
    local param=$1
    load_param $server_conf $param
    echo $RESULT
}